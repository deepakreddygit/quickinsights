...
/**
 * Dev: leave baseURL empty and rely on Vite proxy => no CORS locally
 * Prod: use absolute API URL from VITE_API_URL (Render)
 */
const isDev = import.meta.env.DEV;
const baseURL = isDev ? "" : (import.meta.env.VITE_API_URL as string | undefined) || "";
...
// ---- Auth helpers (JWT in localStorage, sent via Authorization header) ----
export function setToken(t?: string) {
  if (t) localStorage.setItem("token", t);
  else localStorage.removeItem("token");
